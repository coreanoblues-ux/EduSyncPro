...
router.post("/webhooks", async (req: Request, res: Response) => {
  ...
  if (!sig.ok) {
    ...
    // 서명 실패 = 401. 공식 문서가 지정한 응답이다:
    //   "x-toss-signature 를 서명 생성 규칙에 따라 검증하여 Payload 위변조 및 위장 요청을
    //    방지합니다. 서명 검증 실패 시 401 또는 400 으로 응답합니다."
    //
    // ⚠️ 여기 오래 적혀 있던 "401 이면 토스가 재발송을 멈춘다" 는 사실이 아니다.
    //    문서는 정반대를 말한다 — "네트워크 오류, 타임아웃, 3xx/4xx/5xx 응답은 실패로
    //    간주하고 재시도" 하며 전달 정책은 at-least-once 다. 성공은 2xx 뿐이다.
    //
    //    그리고 그게 우리에게 유리하다. 시크릿이 아직 없거나 어긋난 동안 온 웹훅은
    //    토스가 다시 보내 주므로, 시크릿을 제대로 넣는 순간 재전송분이 통과한다.
    //    (재전송은 x-toss-webhook-id 가 같아서 위쪽 중복 분기가 안전하게 받아낸다.)
